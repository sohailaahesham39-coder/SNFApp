...
const DONE = [true,true,false,false,false,false,false];
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.weekCard,{backgroundColor:C.card,borderColor:C.border}]}>
          ...
          <View style={s.weekRow}>
            {DAYS.map((d,i)=>(
              <View key={d} style={[s.dayItem,{backgroundColor:DONE[i]?'rgba(232,255,77,0.08)':C.card,borderColor:DONE[i]?'rgba(232,255,77,0.2)':C.border}]}>
                <Text style={[s.dayLbl,{color:DONE[i]?'#E8FF4D':C.textDim}]}>{d}</Text>
                {DONE[i]&&<Text style={s.check}>✓</Text>}
              </View>
            ))}
          </View>
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.filters}>
          {DIFFS.map(d=>(
            <TouchableOpacity key={d} style={[s.pill,{backgroundColor:diff===d?'#E8FF4D':C.card,borderColor:diff===d?'#E8FF4D':C.border}]} onPress={()=>setDiff(d)}>
              <Text style={[s.pillT,{color:diff===d?'#000':C.textMuted}]}>{d}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
          ...
          <TouchableOpacity key={w.id} style={[s.card,{backgroundColor:C.card,borderColor:C.border}]} onPress={() => router.push({ pathname: '/workout-detail', params: { id: w.id } })}>
            <View style={s.cardTop}>
              ...
              <View style={s.calsBurned}>
                <Text style={s.calsBurnedN}>{w.calories_burned}</Text>
...
const s = StyleSheet.create({
  ...
  calsBurnedN:{fontSize:18,fontWeight:'900',color:'#E8FF4D'},
